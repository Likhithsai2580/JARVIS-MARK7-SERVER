try:
    from llm.base import LLM, Model, ModelType, Role
except ImportError:
    import os
    import sys
    
    sys.path.append(os.path.dirname(__file__))
    from base import LLM, Model, ModelType, Role

from typing import Optional, List, Dict, Generator, AsyncGenerator, Any
from dotenv import load_dotenv
from rich import print
from copy import deepcopy

import os
import together
import asyncio

load_dotenv()

LLAMA_VISION_FREE = Model(name="meta-llama/Llama-Vision-Free", typeof=ModelType.textandimage)

# Add more models
LLAMA_2_70B = Model(name="meta-llama/Llama-2-70b-chat", typeof=ModelType.textonly)
LLAMA_2_13B = Model(name="meta-llama/Llama-2-13b-chat", typeof=ModelType.textonly)
LLAMA_2_7B = Model(name="meta-llama/Llama-2-7b-chat", typeof=ModelType.textonly)
MIXTRAL_8X7B = Model(name="mistralai/Mixtral-8x7B-Instruct-v0.1", typeof=ModelType.textonly)
MISTRAL_7B = Model(name="mistralai/Mistral-7B-Instruct-v0.2", typeof=ModelType.textonly)
CODELLAMA_70B = Model(name="togethercomputer/CodeLlama-70b-Instruct", typeof=ModelType.textonly)
CODELLAMA_34B = Model(name="togethercomputer/CodeLlama-34b-Instruct", typeof=ModelType.textonly)
CODELLAMA_13B = Model(name="togethercomputer/CodeLlama-13b-Instruct", typeof=ModelType.textonly)

class Together(LLM):
    def __init__(
        self,
        model: Model,
        apiKey: Optional[str] = None,
        messages: Optional[List[Dict[str, str]]] = None,
        temperature: float = 0.0,
        systemPrompt: Optional[str] = None,
        maxTokens: int = 2048,
        cheatCode: Optional[str] = None,
        logFile: Optional[str] = None,
        extra: Dict[str, str] = {},
    ):
        messages = messages if messages is not None else []
        super().__init__(model, apiKey, messages, temperature, systemPrompt, maxTokens, logFile)
        
        self.extra = extra
        self.cheatCode = cheatCode
        self.client = None
        
        if cheatCode is None:
            asyncio.create_task(self._init_client())
    
    async def _init_client(self):
        """Initialize the Together client asynchronously"""
        self.client = await self.constructClient()
        if self.client:
            is_valid = await self.testClient()
            if is_valid:
                self.logger.info("Test successful for Together API key. Model found.")
    
    async def constructClient(self) -> Any:
        """Construct Together client asynchronously"""
        try:
            client = together.Together(
                api_key=os.environ["TOGETHER_API_KEY"] if self.apiKey is None else self.apiKey,
            )
            return client
        except Exception as e:
            self.logger.error(f"Failed to construct client: {str(e)}")
            return None
    
    async def testClient(self) -> bool:
        """Test client connection asynchronously"""
        if not self.client:
            return False
            
        try:
            modelListResponse = await asyncio.to_thread(self.client.models.list)
            models = modelListResponse
            
            for modelinfo in models:
                if modelinfo.id == self.model.name:
                    return True
                    
            self.logger.error("Model not found")
            raise Exception("Model not found in Together, please add it to the code.")
        except Exception as e:
            self.logger.error(f"Client test failed: {str(e)}")
            return False
    
    async def run(self, prompt: str = "", imageUrl: Optional[str] = None, save: bool = True) -> str:
        """Run completion asynchronously"""
        if not self.client:
            await self._init_client()
            
        toSend = []
        if save and prompt:
            self.addMessage(Role.user, prompt, imageUrl)
        elif not save and prompt:
            toSend.append(self.getMessage(Role.user, prompt, imageUrl))

        try:
            extra = {}
            if self.cheatCode is not None:
                extra["seed"] = 0

            chat_completion = await asyncio.to_thread(
                self.client.chat.completions.create,
                messages=self.messages + toSend,
                model=self.model.name,
                temperature=self.temperature,
                max_tokens=self.maxTokens,
                **extra,
                **self.extra
            )
        except Exception as e:
            self.logger.error(f"Completion failed: {str(e)}")
            return f"Error: {str(e)}"

        log_completion = deepcopy(chat_completion)
        log_completion.choices[0].message.content = log_completion.choices[0].message.content[:20]
        self.logger.info(log_completion)

        if save:
            self.addMessage(Role.assistant, chat_completion.choices[0].message.content)

        return chat_completion.choices[0].message.content
    
    async def streamRun(self, prompt: str = "", imageUrl: Optional[str] = None, save: bool = True) -> AsyncGenerator[str, None]:
        """Stream completions asynchronously"""
        if not self.client:
            await self._init_client()
            
        toSend = []
        if save and prompt:
            self.addMessage(Role.user, prompt, imageUrl)
        elif not save and prompt:
            toSend.append(self.getMessage(Role.user, prompt, imageUrl))

        max_retries = 3
        retry_delay = 1
        last_error = None
        
        for attempt in range(max_retries):
            try:
                extra = {}
                if self.cheatCode is not None:
                    extra["seed"] = 0

                chat_completion = await asyncio.to_thread(
                    self.client.chat.completions.create,
                    messages=self.messages + toSend,
                    model=self.model.name,
                    temperature=self.temperature,
                    max_tokens=self.maxTokens,
                    stream=True,
                    **extra,
                    **self.extra
                )
                break
            except Exception as e:
                last_error = e
                if attempt == max_retries - 1:
                    self.logger.error(f"Failed after {max_retries} attempts: {str(e)}")
                    raise
                await asyncio.sleep(retry_delay * (attempt + 1))

        final_response = ""
        try:
            async for completion in chat_completion:
                if completion.usage is not None:
                    self.logger.info(completion)
                    break

                if completion.choices[0].delta is not None:
                    chunk = completion.choices[0].delta.content
                    if chunk:
                        final_response += chunk
                        yield chunk
        except Exception as e:
            self.logger.error(f"Error during streaming: {str(e)}")
            raise

        if save and final_response:
            self.addMessage(Role.assistant, final_response)

if __name__ == "__main__":
    llm = Together(LLAMA_VISION_FREE)
    sresp = list(llm.streamRun("what is 2+2 reply in 1 char"))
    
    llm.addMessage("user", "Hello, how are you?")
    llm.addMessage("assistant", "I'm doing well, thank you!")
    r = llm.run("what is in the image", "data:image/webp;base64,")
    # print(llm.messages)
    print(r)
