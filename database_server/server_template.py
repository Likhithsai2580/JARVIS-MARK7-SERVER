...
from .logger_client import DatabaseLoggerClient
import traceback
...
class BaseServer:
    def __init__(self, service_name: str):
        ...
        self.logger = DatabaseLoggerClient(
            server_name=service_name,
            database_url=os.getenv("DATABASE_SERVER_URL", "http://localhost:5006")
        )
        ...
        # Add startup and shutdown events for logger
        @self.app.on_event("startup")
        async def startup_event():
            await self.register_with_dns()
            await self.logger.connect()
            await self.logger.log(
                message=f"{service_name} server started",
                log_type="info",
        ...
        async def shutdown_event():
            await self.logger.log(
                message=f"{service_name} server shutting down",
                log_type="info"
            )
            if self.dns_client:
